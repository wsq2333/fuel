...
**禁止**返回第三种状态：缓存层无法证明正确性、却又未回源，把陈旧/损坏/不可信的数据当作正确数据返回给应用。换言之，缓存层"不知道自己对不对"时，唯一合法动作是 miss 回源，绝不可将就返回。

- 缓存命中后必须能用 ETag / 内容校验等机制证明数据正确；证明不了就当 miss
- ETag 不匹配（对象已被外部修改）→ 立即剔除缓存条目并回源，不得返回旧内容
- 磁盘 I/O 错误、文件损坏、半拉取残留 → 跳过缓存直透对象存储，不得返回错误数据
- 元数据 TTL 过期或失效 → 必须回源重查，不得返回陈旧元数据
- 缓存层任何"不确定"状态（校验失败、并发竞争、部分写入）→ 一律 miss 回源
- 不存在"缓存层返回了数据但不知是否正确"的灰色路径
...
1. **不变量合规检查** — 生成代码前，确认代码不违反任何不变量:
   ...
   - INV-9: 缓存命中后是否校验了正确性（ETag/内容 MD5）？校验失败/不确定时是否回源？若返回了未校验或陈旧数据，违反 INV-9
...
| 缓存命中未校验就返回数据 | 违反 INV-9 |
| 缓存校验失败/不确定时未回源 | 违反 INV-9 |
| 返回陈旧/损坏数据冒充正确 | 违反 INV-9 |
